...
namespace OneOf.Serialization
{
    ...
    public class OneOfStructJsonConverter<T0, T1> : JsonConverter
    {
        private static readonly Type OneOfType = typeof(OneOf<T0, T1>);
        ...
        {
            if (value is OneOf<T0, T1> v)
            {
                serializer.Serialize(writer, v.Value);
            }
            else if (value is IEnumerable<OneOf<T0, T1>> en)
            {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
                     ...
                     OneOfJsonConverterCommon.CanConvertByEnumerating(objectType, OneOfType))
            {
                ...
                var oneOfList = new List<OneOf<T0, T1>> (jArray.Count);
        ...
        private OneOf<T0, T1>? DeserializeToOneOf(JToken jToken)
        {
        ...
        private OneOf<T0, T1> DeserializeToOneOfWithStringOrEnumArg(string oneOfArg, int oneOfArgIndex)
        {
        ...
        private OneOf<T0, T1> CreateOneOfInstance(object oneOfArg, int oneOfArgIndex)
        {
            Debug.Assert((uint) oneOfArgIndex < OneOfArgTypes.Length);
            return
                (oneOfArgIndex == 0) ? (OneOf<T0, T1>)(T0)oneOfArg :
                (OneOf<T0, T1>)(T1)oneOfArg;
        }
